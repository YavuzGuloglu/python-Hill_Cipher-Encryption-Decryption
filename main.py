alphabet = ["a", "b", "c", "ç", "d", "e", "f", "g", "ð", "h", "ý", "i", "j", "k", "l", "m", "n", "o", "ö", "p", "r",
                "s", "þ", "t", "u", "ü", "v", "y", "z"] 
def findIndex(char):
    index = alphabet.index(char)
    return index

print("2x2 Boyutunda bir anahtar matrisi oluþturun.-->")
matrix = [[0, 0], [0, 0]]
i, j = 0, 0
for i in range(2):
    for j in range(2):
        num = int(input("%d.Satýr %d.Sütunu Girin: " % (i + 1, j + 1)))
        matrix[i][j] = num

#ENCRYPT KODLARI-------------------------------------------------------->
password = input("Þifrelenecek metni girin: ").lower().replace(" ","")
...
if(count%2 != 0):
    extra = input("Ýkili denkleþtirme için ekstra karakteri seçin: ")
...
if(extra!=""):
    ...
    encryptedText+=alphabet[charIndex2]
print("------------------------------------------------------------")
print("ÞÝFRELENECEK METÝN ==>",password+extra,"<==")
print("ÞÝFRELENMÝÞ HALÝ==>",encryptedText,"<==")
...
for i in encryptedText:
    if(control==0):
        ...
        control = 0
print("ÞÝFRENÝN ÇÖZÜLMÜÞ HALÝ==>",password,"<==")
print("------------------------------------------------------------")
